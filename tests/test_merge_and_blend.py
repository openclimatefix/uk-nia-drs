...
def test_blend_data_with_blending(sample_data_xg, sample_data_pvnet):
    ...
    within_range = result[
        (result["Init Time"] >= start_date) & (result["Init Time"] <= end_date)
    ]
    ...
    for _, row in within_range.iterrows():
        ...
        row_pvnet = sample_data_pvnet[
            sample_data_pvnet["Init Time"] == current_time
        ].iloc[0]

        # Calculate expected values based on the matching rows and blending ratios
        expected_value_7 = (row_pvnet["7 Hour Forecast"] * 0.75) + (
            row_xg["7 Hour Forecast"] * 0.25
        )
        expected_value_7_5 = (row_pvnet["7.5 Hour Forecast"] * 0.5) + (
            row_xg["7.5 Hour Forecast"] * 0.5
        )
        expected_value_8 = (row_pvnet["8 Hour Forecast"] * 0.25) + (
            row_xg["8 Hour Forecast"] * 0.75
        )

        assert (
            row["6.5 Hour Forecast"] == row_pvnet["6.5 Hour Forecast"]
        ), f"Mismatch in '6.5 Hour Forecast' for Init Time {current_time}"
        ...
        assert (
            row["8.5 Hour Forecast"] == row_xg["8.5 Hour Forecast"]
        ), f"Mismatch in '8.5 Hour Forecast' for Init Time {current_time}"
    ...
    for _, row in before_start.iterrows():
        ...
        assert (
            row["6.5 Hour Forecast"] == row_xg["6.5 Hour Forecast"]
        ), f"Mismatch in '6.5 Hour Forecast' for Init Time {current_time}"
        assert (
            row["8 Hour Forecast"] == row_xg["8 Hour Forecast"]
        ), f"Mismatch in '8 Hour Forecast' for Init Time {current_time}"
        assert (
            row["8.5 Hour Forecast"] == row_xg["8.5 Hour Forecast"]
        ), f"Mismatch in '8.5 Hour Forecast' for Init Time {current_time}"
